...
class ESO():
	...
	def feso(self):
		...
		self.z2 += self.z3 - self.beta02 * self.fe + self.b * self.u
		self.z3 += -self.beta03*self.fe1
...
def setup():
	...
	#Open DB file
	conn = sqlite3.connect('/home/peng/Documents/2021-01-15-15-37-05.db')
	cursor = conn.cursor()

	cursor.execute("SELECT pitch_desired FROM PID_TUNING;")
	contrl = cursor.fetchall()
	...
	Y_TD_fe  = np.array(range(size), dtype = float)
	Y_TD_fe1 = np.array(range(size), dtype = float)
	...
	col = 1
	fig = plt.figure()
	graph_display(fig,row,col,1,X,Y_TD_y,"eso y",'blue','-')
	graph_display(fig,row,col,1,X,Y_TD_z1,"eso z1",'red','-')
	graph_display(fig,row,col,2,X,Y_TD_u,"eso u",'red','--')
